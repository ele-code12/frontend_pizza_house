...
;(function (d) {
  function r(e, c, l, b, a) {
    function f() {
      ...
      a.checkSort = []
      v.removeStyle(
        a.prefix + 'filter, filter, ' + a.prefix + 'transform, transform, opacity, display'
      )
        .css(a.clean)
        .removeAttr('data-checksum')
      window.atob || v.css({ display: 'none', opacity: '0' })
      n.removeStyle(
        a.prefix +
      ...
      'list' == a.layoutMode
        ? (p.css({ display: a.targetDisplayList, opacity: '1' }),
          (a.origDisplay = a.targetDisplayList))
        : (p.css({ display: a.targetDisplayGrid, opacity: '1' }),
    ...
    Array.prototype.indexOf &&
      ((a.fade = -1 < g.indexOf('fade') ? '0' : ''),
      (a.scale = -1 < g.indexOf('scale') ? 'scale(.01)' : ''),
      (a.rotateZ = -1 < g.indexOf('rotateZ') ? 'rotate(180deg)' : ''),
      (a.rotateY = -1 < g.indexOf('rotateY') ? 'rotateY(90deg)' : ''),
      (a.rotateX = -1 < g.indexOf('rotateX') ? 'rotateX(90deg)' : ''),
      (a.blur = -1 < g.indexOf('blur') ? 'blur(8px)' : ''),
      (a.grayscale = -1 < g.indexOf('grayscale') ? 'grayscale(100%)' : ''))
    var p = d(),
      s = d(),
      t = [],
      r = !1
    ...
    'or' == a.filterLogic
      ...
      : ((p = p.add(n.find(w + '.' + t.join('.')))),
        (s = s.add(n.find(w + ':not(.' + t.join('.') + '):visible'))))
    e = p.length
    var u = d(),
      q = d(),
      m = d()
      ...
      if (1 == p.length)
        return (
          'list' == a.layoutMode
            ...
            : (b.addClass(a.gridClass),
              b.removeClass(a.listClass),
    ...
    if (p.length) {
      ...
      'list' == a.layoutMode
        ? (b.addClass(a.listClass),
          b.removeClass(a.gridClass),
          q.css('display', a.targetDisplayList))
        : (b.addClass(a.gridClass),
          b.removeClass(a.listClass),
      ...
      'block' == a.origDisplay
        ? (b.addClass(a.listClass), q.css('display', a.targetDisplayList))
        : (b.removeClass(a.listClass), q.css('display', a.targetDisplayGrid))
      a.resizeContainer && n.css('height', a.origHeight + 'px')
      e = {}
      for (g = 0; 2 > g; g++)
        (h = 0 == g ? (h = a.prefix) : ''),
          (e[h + 'transform'] = a.scale + ' ' + a.rotateX + ' ' + a.rotateY + ' ' + a.rotateZ),
          (e[h + 'filter'] = a.blur + ' ' + a.grayscale)
      q.css(e)
      m.each(function () {
        var b = this.data,
          c = d(this)
      ...
      'list' == a.layoutMode
        ? (b.addClass(a.listClass), b.removeClass(a.gridClass))
        : (b.addClass(a.gridClass), b.removeClass(a.listClass))
      setTimeout(function () {
        ...
        q.each(function () {
          ...
          d(this)
            .css('-webkit-transition', 'all ' + k + 'ms ' + a.easingFallback)
            .css(c)
        })
        m.each(function () {
          var b = this.data
        ...
        n.bind('webkitTransitionEnd transitionend otransitionend oTransitionEnd', function (a) {
          if (
            -1 < a.originalEvent.propertyName.indexOf('transform') ||
            -1 < a.originalEvent.propertyName.indexOf('opacity')
          )
            -1 < w.indexOf('.')
              ? d(a.target).hasClass(w.replace('.', '')) && f()
              : d(a.target).is(w) && f()
        })
      }, 10)
      ...
      setTimeout(function () {
        ...
        if (s.length) {
          ...
          for (e = 0; 2 > e; e++)
            (d = 0 == e ? (d = a.prefix) : ''),
              (c[d + 'transform'] = a.scale + ' ' + a.rotateX + ' ' + a.rotateY + ' ' + a.rotateZ),
              (c[d + 'filter'] = a.blur + ' ' + a.grayscale),
              (c.opacity = a.fade)
          u.css(c)
  ...
  function x(e, c, l, b) {
    ...
    b.origSort.length ||
      k.find(b.targetSelector + ':visible').each(function () {
        d(this).wrap('<s/>')
    ...
    if ('reset' == e)
      ...
      b.newOrder.length || (b.newOrder = g(b.startOrder)),
  ...
  var y = {
      ...
      return this.each(function () {
        ...
        if (b.sortOnLoad) {
          ...
          d.isArray(b.sortOnLoad)
            ...
            : (d(b.sortSelector + '[data-sort=' + b.sortOnLoad + ']').addClass('active'),
              (c = b.sortOnLoad),
              (b.sortOnLoad = 'desc'))
          x(c, f, a, b)
        }
        for (f = 0; 2 > f; f++)
          (c = 0 == f ? (c = b.prefix) : ''),
            (b.transition[c + 'transition'] = 'all ' + b.transitionSpeed + 'ms ease-in-out'),
            (b.perspective[c + 'perspective'] = b.perspectiveDistance + 'px'),
            (b.perspective[c + 'perspective-origin'] = b.perspectiveOrigin)
        for (f = 0; 2 > f; f++)
          (c = 0 == f ? (c = b.prefix) : ''), (b.clean[c + 'transition'] = 'none')
        'list' == b.layoutMode
          ? (a.addClass(b.listClass), (b.origDisplay = b.targetDisplayList))
          : (a.addClass(b.gridClass), (b.origDisplay = b.targetDisplayGrid))
        ...
        d(b.filterSelector).bind(b.buttonEvent, function () {
          if (!b.mixing) {
            var c = d(this)
            if (!1 == b.multiFilter)
              d(b.filterSelector).removeClass('active'),
                c.addClass('active'),
                (b.filter = c.attr('data-filter')),
                d(b.filterSelector + '[data-filter="' + b.filter + '"]').addClass('active')
            else {
              var e = c.attr('data-filter')
              c.hasClass('active')
                ? (c.removeClass('active'),
                  (b.filter = b.filter.replace(RegExp('(\\s|^)' + e), '')))
                : (c.addClass('active'), (b.filter = b.filter + ' ' + e))
            }
    ...
    filter: function (e) {
      return this.each(function () {
        var c = this.config
        c.mixing ||
          (d(c.filterSelector).removeClass('active'),
          d(c.filterSelector + '[data-filter="' + e + '"]').addClass('active'),
          r(e, null, null, d(this), c))
      })
    },
    sort: function (e) {
      return this.each(function () {
        ...
        if (!c.mixing) {
          ...
          if (d.isArray(e)) {
            var b = e[0],
              a = e[1]
            d(c.sortSelector + '[data-sort="' + e[0] + '"][data-order="' + e[1] + '"]').addClass(
              'active'
            )
          } else
    ...
    multimix: function (e) {
      return this.each(function () {
        var c = this.config,
          l = d(this)
        multiOut = { filter: c.filter, sort: null, order: 'desc', layoutMode: c.layoutMode }
        d.extend(multiOut, e)
        c.mixing ||
          (d(c.filterSelector).add(c.sortSelector).removeClass('active'),
          d(c.filterSelector + '[data-filter="' + multiOut.filter + '"]').addClass('active'),
          ...
          (c.layoutMode = multiOut.layoutMode),
          r(multiOut.filter, multiOut.sort, multiOut.order, l, c))
      })
    },
    remix: function (e) {
      return this.each(function () {
